...
export const updateUserData = async (req, res) => {
    try {
        const { userId } = req.auth()
        let { username, bio, location, full_name, social_media } = req.body
        ...
        const updatedData = {
            ...
            social_media: social_media ? JSON.parse(social_media) : tempUser.social_media
        }
...
export const sendConnectionRequest = async (req, res) => {
    try {
        ...
        if(requests.length >= 20)
            return res.json({ success: false, message: 'Connection request limit reached (20/day)' })
...
export const getUserConnections = async (req, res) => {
    try {
        ...
        const pendingConnections = (await Connection.find({ to_user_id: userId, status: 'pending' })
            .populate('from_user_id')).map(c => c.from_user_id)
        res.json({ success: true, connections: user.connections,
            followers: user.followers, following: user.following, pendingConnections })
    } catch (error) {
        res.json({ success: false, message: error.message })
    }
}
